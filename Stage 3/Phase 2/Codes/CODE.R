...
cholera_data <- cholera_data %>%
  mutate(across(-Countries, ~replace_na(as.numeric(.), 0)))  # Convert all columns except 'Countries' to numeric and replace NAs

# Clean data: remove rows with missing values (optional)
...
reported_cases_per_year <- cholera_data_clean %>%
  mutate(`Number of reported cases of cholera` = as.numeric(`Number of reported cases of cholera`)) %>%  # Ensure cholera cases are numeric
  group_by(Year) %>%
  summarise(total_reported_cases = sum(`Number of reported cases of cholera`, na.rm = TRUE))  # Sum cases per year, handling NA
...
reported_cases_per_country <- cholera_data_clean %>%
  mutate(`Number of reported cases of cholera` = as.numeric(`Number of reported cases of cholera`)) %>%  # Convert to numeric
  group_by(Countries) %>%
  summarise(total_reported_cases = sum(`Number of reported cases of cholera`, na.rm = TRUE)) %>%  # Sum cases per country
...
reported_death_per_year <- cholera_data_clean %>%
  mutate(`Number of reported deaths from cholera` = as.numeric(`Number of reported deaths from cholera`)) %>%  # Convert to numeric
  group_by(Year) %>%
  summarise(total_death_cases = sum(`Number of reported deaths from cholera`, na.rm = TRUE))  # Sum total deaths per year
...
fatality_rate_per_year <- cholera_data_clean %>%
  mutate(`Cholera case fatality rate` = as.numeric(`Cholera case fatality rate`)) %>%  # Convert to numeric
  group_by(Year) %>%
  summarise(avg_fatality_rate = mean(`Cholera case fatality rate`, na.rm = TRUE))  # Calculate average fatality rate
...
fatality_rate_per_country <- cholera_data_clean %>%
  ...
  filter(!is.na(`Cholera case fatality rate`)) %>%  # Remove NAs
  group_by(Countries) %>%
...
top_countries_F <- fatality_rate_per_country %>%
  top_n(20, avg_fatality_rate)  # Adjusting to get the top 20
...
cholera_long_all <- cholera_summary_per_country %>%
  pivot_longer(cols = c(total_cases, total_deaths),  # Use the correct column names
               names_to = "Metric", 
               values_to = "Count") %>%
  mutate(Metric = factor(Metric, levels = c("total_cases", "total_deaths")))  # Ensure total_cases comes first
...
ggplot(cholera_long_all, aes(x = reorder(Countries, -Count), y = Count, fill = Metric)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +  # Dodge bars side by side
  ...
  scale_fill_manual(values = c("total_cases" = "blue", "total_deaths" = "red"), 
                    labels = c("Total Cases", "Total Deaths")) +  # Set colors and labels
  theme_minimal() +
...
cholera_summary_top20 <- cholera_data_clean %>%
  ...
  arrange(desc(total_cases)) %>%   # Sort by total cases
  top_n(20, total_cases)           # Select top 20 countries
...
ggplot(cholera_long_Nigeria, aes(x = Year, y = Count, fill = Metric)) +
  geom_bar(stat = "identity") +  # Use identity to plot values
  ...
  scale_fill_manual(values = c("Total_Reported_Cases" = "blue", "Total_Deaths" = "red"), 
                    labels = c("Total Reported Cases", "Total Deaths")) +  # Set colors and labels
  theme_minimal() +
  theme(axis.text.y = element_text(size = 10), 
...
ggplot(cholera_long, aes(x = Year, y = Countries, fill = Count)) +
  ...
  scale_fill_viridis_c(option = "plasma", name = "Count") +  # Use a color palette
...
ggplot(cholera_long %>% filter(Metric == "Total_Cases"), 
  ...
  geom_tile(color = "white") +  # Add white grid lines
  scale_fill_viridis_c(option = "plasma", name = "Total Cases") +  # Use a color palette
...
ggplot(cholera_long %>% filter(Metric == "Total_Deaths"), 
       aes(x = Year, y = Countries, fill = Count)) +
  geom_tile(color = "white") +  # Add white grid lines
  scale_fill_viridis_c(option = "plasma", name = "Total Death Cases") +  # Use a color palette
...
cholera_long <- cholera_data_clean %>%
  select(Year, Countries, `Cholera case fatality rate`) %>%  # Use the column for average fatality rate
...
ggplot(cholera_long, 
  ...
  scale_fill_viridis_c(option = "plasma", 
                     ...
                     breaks = seq(0, max(cholera_long$Count, na.rm = TRUE), by = 50)) +  # Set breaks to 50
...
ggplot(cholera_70_long, 
  ...
  scale_fill_viridis_c(option = "plasma", 
                       name = "Avg Fatality Rate (%)", 
                       breaks = seq(0, max(cholera_long$Count, na.rm = TRUE), by = 20)) +  # Set breaks to 50
